    ...
    async function fetchResults(evolutionId) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                evolutionResultsDisplay.innerHTML = `<p class="error-message">Error fetching results (HTTP ${response.status}): ${errorMsg}</p>`;
                return;
            }
            
            let resultsHTML = '<h3>Evolution Results:</h3>';
            if (results.result) { 
                if (results.result.output_dir) {
                    resultsHTML += `<p><strong>Output Directory:</strong> ${results.result.output_dir}</p>`;
                }
                if (results.result.stdout) {
                    resultsHTML += `<p><strong>Standard Output:</strong></p><pre>${results.result.stdout}</pre>`;
                }
                if (results.result.stderr) {
                    resultsHTML += `<p><strong>Standard Error:</strong></p><pre>${results.result.stderr}</pre>`;
                }
                if (bestProgramCodeElement) {
                 ...
                 resultsHTML += '<p>No detailed result object was returned by the server.</p>';
            }
            
            const noSpecificResults = !results.result || 
                                     (Object.keys(results.result).length === 0 && !results.result?.best_program_content);
            if (noSpecificResults) {
                 resultsHTML += '<p>No specific result data was found for this evolution.</p>';
            }
            evolutionResultsDisplay.innerHTML = resultsHTML;

        } catch (error) { 
            console.error('Error fetching results:', error);
            evolutionResultsDisplay.innerHTML = `<p class="error-message">An application error occurred while fetching results: ${error.message}</p>`;
        }
    }
    ...
    async function pollStatus(evolutionId) {
        ...
        try {
            ...
            if (statusData.status === 'completed') {
                ...
                if (statusData.result?.stderr) errorMsg += `\nStderr: ${statusData.result.stderr}`;
                if (statusData.result?.stdout && !statusData.result?.stderr) errorMsg += `\nStdout: ${statusData.result.stdout}`;
                
                showUserMessage(evolutionStatusText, errorMsg, 'error'); 
                if (evolutionResultsDisplay) { 
                     evolutionResultsDisplay.innerHTML = `<p class="error-message"><strong>Error during evolution:</strong><br>${errorMsg.replace(/\n/g, '<br>')}</p>`;
                }
                runEvolutionButton.disabled = false;
            } else if (['running', 'preparing', 'starting'].includes(statusData.status)) {
